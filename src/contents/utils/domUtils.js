...
function buildTreeRecursion(
  ...
  while (queue.length > 0) {
    ...
    for (let i = 0; i < currentFrame.length; i++) {
      queue.push(currentFrame[i])
      // console.log("🚀 ~ queue:", queue)
    }
    try {
      const uniqueID = currentFrame.frameElement?.getAttribute("unique_id") || "main.frame"
      // console.log("🚀 ~ frames:", currentFrame)
      // console.log("🚀 ~ uniqueID:", uniqueID)
      const [elementsFrame, elementTreeFrame] = buildElementTree(
        currentFrame.document.body,
        uniqueID
      )

      for (const element of elements) {
        if (element["id"] === uniqueID) {
          element["children"] = elementsFrame
        }
      }

      for (const elementTreeItem of elementTree) {
        if (elementTreeItem["id"] === uniqueID) {
          elementTreeItem["children"] = elementTreeFrame
        }
      }
      elements.push(...elementsFrame)
    } catch (e) {
      continue
    }
...
export { buildTreeFromBody, buildTreeRecursion }